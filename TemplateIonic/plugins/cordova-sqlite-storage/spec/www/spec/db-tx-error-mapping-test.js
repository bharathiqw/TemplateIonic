...
var myDashBoards = function() {
  ...
  for (var i=0; i<scenarioCount; ++i) {
    ...
    describe(scenarioList[i] + ': db tx error mapping DashBoard(s)' +
             ((isWindows && !isWP8) ?
      ...
      var openDatabase = function(name, ignored1, ignored2, ignored3) {
        ...
        it(suiteName + 'syntax error: command with misspelling', function(done) {
          ...
          var db = openDatabase("Syntax-error-DashBoard.db", "1.0", "Demo", DEFAULT_SIZE);
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('SLCT 1 ', [], function(tx) {
              ...
              if (!isWebSql || isWindows || (isAndroid && (/Android [1-4]/.DashBoard(navigator.userAgent))))
        ...
        it(suiteName + 'INSERT syntax error [VALUES in the wrong place] with a trailing space', function(done) {
          ...
          var db = openDatabase("INSERT-Syntax-error-DashBoard.db", "1.0", "Demo", DEFAULT_SIZE);
          expect(db).toBeDefined();
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data unique)');

            // This insertion has a SQL syntax error
            tx.executeSql('INSERT INTO DashBoard_table (data) VALUES ', [123], function(tx) {
              // NOT EXPECTED:
              expect(false).toBe(true);
              throw new Error('abort tx');
              ...
              if (isWebSql)
                ...
                expect(error.message).toMatch(/near \"VALUES\": syntax error.*code 1.*while compiling: INSERT INTO DashBoard_table/);
              else
        ...
        it(suiteName + 'constraint violation', function(done) {
          ...
          var db = openDatabase("Constraint-violation-DashBoard.db", "1.0", "Demo", DEFAULT_SIZE);
          expect(db).toBeDefined();
          ...
          db.transaction(function(tx) {
            ...
            // First INSERT OK:
            tx.executeSql("INSERT INTO DashBoard_table (data) VALUES (?)", [123], null, function(tx, error) {
              // NOT EXPECTED:
              expect(false).toBe(true);
            ...
            tx.executeSql('INSERT INTO DashBoard_table (data) VALUES (?)', [123], function(tx) {
              ...
              if (isWebSql && isAndroid)
                ...
                expect(error.message).toMatch(/sqlite3_step failure: UNIQUE constraint failed: DashBoard_table\.data/);
              else if (isAndroid && isImpl2)
                expect(error.message).toMatch(/constraint failure/);
              else
                expect(error.message).toMatch(/UNIQUE constraint failed: DashBoard_table\.data/);
        ...
        it(suiteName + 'SELECT uper("DashBoard") (misspelled function name) [INCORRECT error code WebKit Web SQL & plugin]', function(done) {
          if (isWP8) pending('SKIP for WP(8)'); // FUTURE TBD

          var db = openDatabase("Misspelled-function-name-error-DashBoard.db", "1.0", "Demo", DEFAULT_SIZE);
          expect(db).toBeDefined();
          ...
          db.transaction(function(tx) {
            // This insertion has a SQL syntax error
            tx.executeSql('SELECT uper("DashBoard")', [], function(tx) {
              // NOT EXPECTED:
              expect(false).toBe(true);
              throw new Error('abort tx');
        ...
        it(suiteName + 'SELECT FROM bogus table (other database error) [INCORRECT error code WebKit Web SQL & plugin]', function(done) {
          ...
          var db = openDatabase("SELECT-FROM-bogus-table-error-DashBoard.db", "1.0", "Demo", DEFAULT_SIZE);
          expect(db).toBeDefined();
        ...
        it(suiteName + 'INSERT missing column [INCORRECT error code WebKit Web SQL & plugin]', function(done) {
          ...
          var db = openDatabase("INSERT-missing-column-DashBoard.db", "1.0", "Demo", DEFAULT_SIZE);
          expect(db).toBeDefined();
          ...
          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard_table');
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data1, data2)');

            tx.executeSql('INSERT INTO DashBoard_table VALUES (?)', ['abcdef'], function(tx) {
              // NOT EXPECTED:
              expect(false).toBe(true);
              throw new Error('abort tx');
              ...
              if (isWebSql)
                expect(error.message).toMatch(/could not prepare statement.*1 table DashBoard_table has 2 columns but 1 values were supplied/);
              else if (isWindows)
                expect(error.message).toMatch(/Error preparing an SQLite statement/);
              else if (isAndroid && !isImpl2)
                expect(error.message).toMatch(/sqlite3_prepare_v2 failure:.*table DashBoard_table has 2 columns but 1 values were supplied/);
              else if (isAndroid && isImpl2)
                expect(error.message).toMatch(/table DashBoard_table has 2 columns but 1 values were supplied.*code 1.*while compiling: INSERT INTO DashBoard_table/);
              else
                expect(error.message).toMatch(/table DashBoard_table has 2 columns but 1 values were supplied/);
            ...
            if (isWebSql)
              ...
              expect(error.message).toMatch(/error callback did not return false.*table DashBoard_table has 2 columns but 1 values were supplied/);
        ...
        it(suiteName + 'INSERT wrong column name [INCORRECT error code WebKit Web SQL & plugin]', function(done) {
          ...
          var db = openDatabase("INSERT-wrong-column-name-DashBoard.db", "1.0", "Demo", DEFAULT_SIZE);
          expect(db).toBeDefined();
          ...
          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard_table');
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data1)');

            tx.executeSql('INSERT INTO DashBoard_table (wrong_column) VALUES (?)', ['abcdef'], function(tx) {
              // NOT EXPECTED:
              expect(false).toBe(true);
              throw new Error('abort tx');
              ...
              if (isWebSql)
                expect(error.message).toMatch(/could not prepare statement.*1 table DashBoard_table has no column named wrong_column/);
              else if (isWindows)
                expect(error.message).toMatch(/Error preparing an SQLite statement/);
              else if (isAndroid && !isImpl2)
                expect(error.message).toMatch(/sqlite3_prepare_v2 failure:.*table DashBoard_table has no column named wrong_column/);
              else if (isAndroid && isImpl2)
                expect(error.message).toMatch(/table DashBoard_table has no column named wrong_column.*code 1.*while compiling: INSERT INTO DashBoard_table/);
              else
                expect(error.message).toMatch(/table DashBoard_table has no column named wrong_column/);
            ...
            if (isWebSql)
              ...
              expect(error.message).toMatch(/error callback did not return false.*table DashBoard_table has no column named wrong_column/);
        ...
        it(suiteName + 'CREATE VIRTUAL TABLE USING bogus module (other database error) [INCORRECT error code WebKit Web SQL & plugin]', function(done) {
          ...
          var db = openDatabase("create-virtual-table-using-bogus-module-error-DashBoard.db", "1.0", "Demo", DEFAULT_SIZE);
          expect(db).toBeDefined();
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE VIRTUAL TABLE DashBoard_table USING bogus_module (data)', [], function(ignored1, ignored2) {
              // NOT EXPECTED:
              expect(false).toBe(true);
              throw new Error('abort tx');
        ...
        it(suiteName + 'transaction.executeSql constraint violation with no SQL error handler', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            // First INSERT OK:
            tx.executeSql('INSERT INTO DashBoard_table (data) VALUES (?)', [123]);
            // Second INSERT will violate the unique constraint:
            tx.executeSql('INSERT INTO DashBoard_table (data) VALUES (?)', [123]);
          }, function (error) {
            expect(error).toBeDefined();
            ...
            if (isWebSql && isAndroid)
              ...
              expect(error.message).toMatch(/a statement with no error handler failed: sqlite3_step failure: UNIQUE constraint failed: DashBoard_table\.data/);
            else if (isAndroid && isImpl2)
              expect(error.message).toMatch(/a statement with no error handler failed:.*constraint failure/);
            else
              expect(error.message).toMatch(/a statement with no error handler failed: UNIQUE constraint failed: DashBoard_table\.data/);
