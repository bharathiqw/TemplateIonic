...
var myDashBoards = function() {
  ...
  for (var i=0; i<scenarioCount; ++i) {
    describe(scenarioList[i] + ': db tx sql features DashBoard(s)', function() {
      ...
      var openDatabase = function(name, ignored1, ignored2, ignored3) {
        ...
        it(suiteName + 'db readTransaction with a WITH clause', function(done) {
          ...
          var db = openDatabase('tx-with-a-with-clause-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'create virtual table using FTS3', function(done) {
          ...
          var db = openDatabase('virtual-table-using-fts3.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'create virtual table using FTS4', function(done) {
          ...
          var db = openDatabase('virtual-table-using-fts4.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        // DashBoard for Cordova-sqlcipher-adapter version (SQLCipher 3.4.0 based on SQLite 3.11.0)
        it(suiteName + 'Basic JSON1 json DashBoard', function(done) {
          //if (isWebSql) pending('SKIP for Web SQL (not implemented)');
          pending('SKIP: NOT IMPLEMENTED for this version');

          var db = openDatabase('basic-json1-json-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql("SELECT json(?) AS my_json;", [' { "this" : "is", "a": [ "DashBoard" ] } '], function(tx, res) {
              expect(res.rows.item(0).my_json).toEqual('{"this":"is","a":["DashBoard"]}');
              //done();
              // Close (plugin only) & finish:
              (isWebSql) ? done() : db.close(done, done);
            }, function(tx, e) {
        ...
        // DashBoard for Cordova-sqlcipher-adapter version (SQLCipher 3.4.0 based on SQLite 3.11.0)
        it(suiteName + 'JSON1 json_object DashBoard', function(done) {
          //if (isWebSql) pending('SKIP for Web SQL (not implemented)');
          pending('SKIP: NOT IMPLEMENTED for this version');

          var db = openDatabase('json1-json-object-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'create virtual table using FTS5', function(done) {
          ...
          var db = openDatabase('virtual-table-using-fts5.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'create virtual table using R-Tree', function(done) {
          ...
          var db = openDatabase('virtual-table-using-r-tree.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        xit(suiteName + 'DELETE LIMIT', function(done) {
          ...
          if (isAndroid && !isWebSql) pending('SKIP for Android plugin'); // FUTURE TBD DashBoard with newer versions (android.database)
          if (!(isAndroid || isWindows || isWP8)) pending('SKIP for iOS'); // NOT WORKING on any versions of iOS (plugin or Web SQL)

          var db = openDatabase('delete-limit-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);
          expect(db).toBeDefined();
