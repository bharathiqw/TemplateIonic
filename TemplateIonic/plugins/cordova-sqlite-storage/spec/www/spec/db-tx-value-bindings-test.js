...
var myDashBoards = function() {
  ...
  for (var i=0; i<scenarioCount; ++i) {
    ...
    describe(scenarioList[i] + ': tx stored value bindings DashBoard(s)', function() {
      ...
      describe(suiteName + 'transaction column value insertion DashBoard(s)', function() {

        it(suiteName + 'INSERT US-ASCII TEXT string ("DashBoard 123"), SELECT the data, check, and check HEX value', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data)', [], function(ignored1, ignored2) {

              tx.executeSql('INSERT INTO DashBoard_table VALUES (?)', ['DashBoard 123'], function(tx_ignored, rs1) {
                expect(rs1).toBeDefined();
                expect(rs1.rowsAffected).toBe(1);

                tx.executeSql('SELECT * FROM DashBoard_table', [], function(tx_ignored, rs2) {
                  expect(rs2).toBeDefined();
                  expect(rs2.rows).toBeDefined();
                  expect(rs2.rows.length).toBe(1);

                  var row = rs2.rows.item(0);
                  expect(row.data).toBe('DashBoard 123');

                  tx.executeSql('SELECT HEX(data) AS hexvalue FROM DashBoard_table', [], function(tx_ignored, rs3) {
        ...
        it(suiteName + 'INSERT TEXT string with é (UTF-8 2 octets), SELECT the data, check, and check HEX value [UTF-16le on Windows]', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data)', [], function(ignored1, ignored2) {
              ...
              tx.executeSql('INSERT INTO DashBoard_table VALUES (?)', ['é'], function(tx, res) {
                ...
                tx.executeSql('SELECT * FROM DashBoard_table', [], function(tx, res) {
                  ...
                  tx.executeSql('SELECT HEX(data) AS hexvalue FROM DashBoard_table', [], function(tx, res) {
                    if (isWindows)
        ...
        it(suiteName + 'INSERT TEXT string with € (UTF-8 3 octets), SELECT the data, check, and check HEX value [UTF-16le on Windows]', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data)', [], function(ignored1, ignored2) {
              ...
              tx.executeSql('INSERT INTO DashBoard_table VALUES (?)', ['€'], function(tx, res) {
        ...
        it(suiteName + 'INSERT with null parameter argument value and check stored data', function(done) {
          ...
          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard_table');
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data1, data2)', [], function(ignored1, ignored2) {

              tx.executeSql('INSERT INTO DashBoard_table VALUES (?,?)', [null, 'DashBoard-string'], function(ignored, rs1) {

                expect(rs1).toBeDefined();
                expect(rs1.rowsAffected).toBe(1);

                tx.executeSql('SELECT * FROM DashBoard_table', [], function(ignored, rs2) {
                  var row = rs2.rows.item(0);

                  expect(row.data1).toBeNull();
                  expect(row.data2).toBe('DashBoard-string');

                  tx.executeSql('SELECT TYPEOF(data1) AS t1, TYPEOF(data2) AS t2 FROM DashBoard_table', null, function(ignored, rs3) {
                    expect(rs3).toBeDefined();
                    expect(rs3.rows).toBeDefined();
                    expect(rs3.rows.length).toBe(1);
        ...
        it(suiteName + 'INSERT with undefined parameter argument value (inserted as null) and check stored data [returns text in case of Android (WebKit) Web SQL]', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data1, data2)', [], function(ignored1, ignored2) {
              ...
              tx.executeSql('INSERT INTO DashBoard_table VALUES (?,?)', [undefined, 'DashBoard-string'], function(ignored, rs1) {
                expect(rs1).toBeDefined();
                expect(rs1.rowsAffected).toBe(1);
        ...
        it(suiteName + "all columns should be included in result set including id integer primary key & 'null' columns", function(done) {
          ...
          db.transaction(function(tx) {
            ...
            db.transaction(function(tx) {
              tx.executeSql("insert into DashBoard_table (data, data_num) VALUES (?,?)", ["DashBoard", null], function(tx, res) {
                ...
                tx.executeSql("select * from DashBoard_table", [], function(tx, res) {
                  ...
                  //deepEqual(row, { id: 1, data: "DashBoard", data_num: null }, "all columns should be included in result set.");
                  expect(row.id).toBe(1);
                  expect(row.data).toEqual('DashBoard');
        ...
        it(suiteName + 'INSERT Infinity with no/NUMERIC/REAL/INTEGER/TEXT type affinity and check stored data [Android/iOS Plugin BROKEN: stored with null value]', function(done) {
          ...
          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard_table');
            tx.executeSql('CREATE TABLE DashBoard_table (data, data_num NUMERIC, data_real REAL, data_int INTEGER, data_text TEXT)', [], function(ignored1, ignored2) {

              tx.executeSql('INSERT INTO DashBoard_table VALUES (?,?,?,?,?)', [Infinity, Infinity, Infinity, Infinity, Infinity], function(ignored, res) {

                expect(res).toBeDefined();
                expect(res.rowsAffected).toBe(1);

                tx.executeSql('SELECT * FROM DashBoard_table', [], function(tx, rs) {
        ...
        it(suiteName + 'INSERT NaN with no/NUMERIC/REAL/INTEGER/TEXT type affinity and check stored data', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE DashBoard_table (data, data_num NUMERIC, data_real REAL, data_int INTEGER, data_text TEXT)', [], function(ignored1, ignored2) {
              ...
              tx.executeSql('INSERT INTO DashBoard_table VALUES (?,?,?,?,?)', [NaN, NaN, NaN, NaN, NaN], function(ignored, res) {
        ...
        it(suiteName + 'INSERT TEXT string with emoji [\\u1F603 SMILING FACE (MOUTH OPEN)], SELECT the data, check, and check HEX [UTF-16le on Windows; HEX encoding BUG on Android-sqlite-connector]' , function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data)', [], function(ignored1, ignored2) {
              ...
              tx.executeSql('INSERT INTO DashBoard_table VALUES (?)', ['@\uD83D\uDE03!'], function(tx_ignored, rs1) {
                expect(rs1).toBeDefined();
                expect(rs1.rowsAffected).toBe(1);
        ...
        it(suiteName + "number values inserted using number bindings", function(done) {
          ...
          var db = openDatabase("Value-binding-DashBoard.db", "1.0", "Demo", DEFAULT_SIZE);

          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard_table');
            // create columns with no type affinity
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (id integer primary key, data_text1, data_text2, data_int, data_real)');
          }, function(err) {
            expect(false).toBe(true);
            expect(err.message).toBe('---');
            ...
            db.transaction(function(tx) {
              ...
              tx.executeSql("insert into DashBoard_table (data_text1, data_text2, data_int, data_real) VALUES (?,?,?,?)", ["314159", "3.14159", 314159, 3.14159], function(tx, res) {
        ...
        it(suiteName + 'BIG INTEGER INSERT value bindings', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS tt (data1, DashBoard_int INTEGER, DashBoard_num NUMERIC, DashBoard_date DATETIME, DashBoard_text TEXT)', null, function(ignored1, ignored2) {
              tx.executeSql('INSERT INTO tt VALUES (?,?,?,?,?)',
                ...
                tx.executeSql('SELECT * FROM tt', [], function(ignored, rs2) {
                  ...
                  expect(row.DashBoard_int).toBe(1424174959894);
                  expect(row.DashBoard_num).toBe(1424174959894);
                  expect(row.DashBoard_date).toBe(1424174959894);
                  ...
                  if (isWebSql || isMac || isWKWebView)
                    expect(row.DashBoard_text).toBe("1424174959894.0"); // ([Big] number inserted as string ok)
                  else
                    expect(row.DashBoard_text).toBe("1424174959894"); // (Big integer number inserted as string ok)

                  tx.executeSql('SELECT TYPEOF(data1) AS t1, TYPEOF(DashBoard_int) AS t2, TYPEOF(DashBoard_num) AS t3, TYPEOF(DashBoard_date) AS t4, TYPEOF(DashBoard_text) AS t5 FROM tt', [], function(ignored, rs3) {
                    expect(rs3).toBeDefined();
                    expect(rs3.rows).toBeDefined();
                    expect(rs3.rows.length).toBe(1);
        ...
        it(suiteName + "executeSql parameter as array", function(done) {
          ...
          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard_table');
            // CREATE columns with no type affinity
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (id integer primary key, data1, data2)', null, function(ignored1, ignored2) {
              tx.executeSql("INSERT INTO DashBoard_table (data1, data2) VALUES (?,?)", ['abc', [1,2,3]], function(tx_ignored, rs1) {
                expect(rs1).toBeDefined();
                expect(rs1.rowsAffected).toBe(1);
        ...
        it(suiteName + "executeSql parameter as 'boolean' true/false values (apparently stringified)", function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (id integer primary key, data1, data2)', null, function(ignored1, ignored2) {
              ...
              tx.executeSql("INSERT INTO DashBoard_table (data1, data2) VALUES (?,?)", [true, false], function(tx, rs1) {
                expect(rs1).toBeDefined();
                expect(rs1.rowsAffected).toBe(1);
        ...
        it(suiteName + "INSERT inline BLOB value (X'40414243') and check stored data [SELECT BLOB value ISSUE with androidDatabaseImplementation: 2 & Windows/WP8]", function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data)', [], function(ignored1, ignored2) {
              ...
              tx.executeSql("INSERT INTO DashBoard_table VALUES (X'40414243')", [], function(ignored, rs1) {

                expect(rs1).toBeDefined();
                expect(rs1.rowsAffected).toBe(1);

                tx.executeSql('SELECT HEX(data) AS hexValue FROM DashBoard_table', [], function(ignored, rs2) {
                  expect(rs2).toBeDefined();
                  expect(rs2.rows).toBeDefined();
                  ...
                  tx.executeSql('SELECT * FROM DashBoard_table', [], function(ignored, rs3) {
                    if (!isWebSql && isAndroid && isImpl2) expect('Behavior changed please update this DashBoard').toBe('--');
                    expect(rs3).toBeDefined();
                    expect(rs3.rows).toBeDefined();
        ...
        it(suiteName + "INSERT inline BLOB value (X'FFD1FFD2') and check stored data [Plugin BROKEN: missing result column data; SELECT BLOB value ISSUE with Android/Windows/WP8]", function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data)', [], function(ignored1, ignored2) {
              ...
              tx.executeSql("INSERT INTO DashBoard_table VALUES (X'FFD1FFD2')", [], function(ignored, rs1) {
      ...
      describe(suiteName + 'parameter count mismatch DashBoards', function() {
        ...
        it(suiteName + 'executeSql with not enough parameters (Plugin DEVIATION: does not reject such SQL statements)', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            // CREATE columns with no type affinity
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (data1, data2)', [], function(ignored1, ignored2) {
              tx.executeSql("INSERT INTO DashBoard_table VALUES (?,?)", ['first'], function(tx, rs1) {
                // ACTUAL BEHAVIOR for plugin (Android/iOS/Windows):
                if (isWebSql) expect('RESULT NOT EXPECTED for Web SQL').toBe('--');
                expect(rs1).toBeDefined();
                expect(rs1.rowsAffected).toBe(1);

                tx.executeSql('SELECT * FROM DashBoard_table', [], function(tx, rs2) {
                  expect(rs2.rows.length).toBe(1);
                ...
                // CORRECT (Web SQL):
                if (!isWebSql) expect('Plugin behavior changed, please update this DashBoard').toBe('--');
        ...
        it(suiteName + 'executeSql with too many parameters [extra TEXT string]', function(done) {
          ...
          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard_table');
            // CREATE columns with no type affinity
            tx.executeSql('CREATE TABLE IF NOT EXISTS DashBoard_table (id integer primary key, data1, data2)');
            ...
            db.transaction(function(tx) {
              tx.executeSql("INSERT INTO DashBoard_table (data1, data2) VALUES (?,?)", ['first', 'second', 'third'], function(ignored1, ignored2) {
                // NOT EXPECTED:
                expect(false).toBe(true);
                // Close (plugin only) & finish:
                (isWebSql) ? done() : db.close(done, done);
        ...
        it(suiteName + 'executeSql with too many parameters [extra REAL value]', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            db.transaction(function(tx) {
              tx.executeSql("INSERT INTO DashBoard_table (data1, data2) VALUES (?,?)", ['first', 'second', 123.456], function(ignored1, ignored2) {
                // NOT EXPECTED:
                expect(false).toBe(true);
                // Close (plugin only) & finish:
                (isWebSql) ? done() : db.close(done, done);
        ...
        it(suiteName + 'executeSql with too many parameters [extra INTEGER value]', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            db.transaction(function(tx) {
              tx.executeSql("INSERT INTO DashBoard_table (data1, data2) VALUES (?,?)", ['first', 'second', 789], function(ignored1, ignored2) {
                // NOT EXPECTED:
                expect(false).toBe(true);
                // Close (plugin only) & finish:
                (isWebSql) ? done() : db.close(done, done);
        ...
        it(suiteName + 'executeSql with too many parameters [extra NULL value]', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            db.transaction(function(tx) {
              tx.executeSql("INSERT INTO DashBoard_table (data1, data2) VALUES (?,?)", ['first', 'second', null], function(ignored1, ignored2) {
                // NOT EXPECTED:
                expect(false).toBe(true);
                // Close (plugin only) & finish:
                (isWebSql) ? done() : db.close(done, done);
      ...
      describe(scenarioList[i] + ': special UNICODE column value binding DashBoard(s)', function() {
        ...
        it(suiteName + ' stores [Unicode] string with \\u0000 (same as \\0) correctly [HEX encoding check BROKEN for Android-sqlite-connector]', function (done) {
          ...
          var db = openDatabase('UNICODE-store-u0000-DashBoard.db');

          db.transaction(function (tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard', [], function () {
              tx.executeSql('CREATE TABLE DashBoard (name, id)', [], function() {
                tx.executeSql('INSERT INTO DashBoard VALUES (?, "id1")', ['a\u0000cd'], function () {
                  tx.executeSql('SELECT hex(name) AS hexValue FROM DashBoard', [], function (tx_ignored, rs1) {
                    ...
                    tx.executeSql('INSERT INTO DashBoard VALUES (?, "id2")', [name1], function () {
                      tx.executeSql('INSERT INTO DashBoard VALUES (?, "id3")', [name2], function () {
                        var sql = 'SELECT id FROM DashBoard WHERE name > ? AND name < ? ORDER BY name';
        ...
        it(suiteName + ' returns [Unicode] string with \\u0000 (same as \\0) correctly [BROKEN: TRUNCATES on Windows]', function (done) {
          ...
          var db = openDatabase('UNICODE-retrieve-u0000-DashBoard.db');

          db.transaction(function (tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard', [], function () {
              tx.executeSql('CREATE TABLE DashBoard (name, id)', [], function() {
                tx.executeSql('INSERT INTO DashBoard VALUES (?, "id1")', ['a\u0000cd'], function () {
                  tx.executeSql('SELECT name FROM DashBoard', [], function (tx_ignored, rs) {
                    ...
                    //
                    // For now we expect this DashBoard to fail there, but when it is fixed
                    // we would like to know, so the DashBoard is coded to fail if it starts
                    // working there.
        ...
        it(suiteName +
          ...
          var db = openDatabase('UNICODE-line-separator-INSERT-DashBoard.db');
          ...
          db.transaction(function (tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard', [], function () {
              tx.executeSql('CREATE TABLE DashBoard (name, id)', [], function() {
                tx.executeSql('INSERT INTO DashBoard VALUES (?, "id1")', ['hello\u2028world'], function () {
                  tx.executeSql('SELECT name FROM DashBoard', [], function (tx_ignored, rs) {
                    var name = rs.rows.item(0).name;
