...
var myDashBoards = function() {
  ...
  for (var i=0; i<pluginScenarioCount; ++i) {
    ...
    describe(pluginScenarioList[i] + ': BATCH SQL DashBoard(s)', function() {
      ...
      describe(pluginScenarioList[i] + ': Basic sql batch DashBoard(s)', function() {

        it(suiteName + 'Single-column batch sql DashBoard', function(done) {
          var db = openDatabase('Single-column-batch-sql-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);
          ...
          db.sqlBatch([
            ...
            db.executeSql('SELECT * FROM MyTable', [], function (rs) {
              ...
              expect(rs.rows.item(0).data).toBe('DashBoard-value');
              db.close(done, done);
            });
          }, function(error) {
            // NOT EXPECTED:
        ...
        it(suiteName + 'Single-column batch sql DashBoard 2 (CREATE TABLE SQL statement with no parameters in [])', function(done) {
          var db = openDatabase('Single-column-batch-sql-DashBoard-2.db', '1.0', 'DashBoard', DEFAULT_SIZE);
          expect(db).toBeDefined();
        ...
        it(suiteName + 'Single-column batch sql DashBoard values: INSERT INTEGER/REAL number values and check stored data', function(done) {
          var db = openDatabase('Single-column-batch-sql-DashBoard-number-values.db', '1.0', 'DashBoard', DEFAULT_SIZE);
          expect(db).toBeDefined();
        ...
        it(suiteName + 'Single-column batch sql DashBoard values: INSERT null/undefined values and check stored data', function(done) {
          var db = openDatabase('Single-column-batch-sql-DashBoard-null-undefined-values.db', '1.0', 'DashBoard', DEFAULT_SIZE);
          expect(db).toBeDefined();
        ...
        it(suiteName + 'Single-column batch sql DashBoard values: INSERT +/- Infinity & NaN values and check stored data [TBD Android/iOS/macOS plugin result for +/- Infinity]', function(done) {
          if (isWP8) pending('SKIP for WP8'); // SKIP for now
          if (isMac) pending('SKIP for macOS [CRASH]'); // FUTURE TBD

          var db = openDatabase('Single-column-batch-sql-DashBoard-infinity-nan-values.db', '1.0', 'DashBoard', DEFAULT_SIZE);
          expect(db).toBeDefined();
        ...
        it(suiteName + 'Single-column batch sql DashBoard values: INSERT true/false values and check stored data [stored as strings]', function(done) {
          var db = openDatabase('Single-column-batch-sql-DashBoard-true-false-values.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'batch sql with dynamic object for SQL [INCONSISTENT BEHAVIOR]', function(done) {
          ...
          var db = openDatabase('batch-sql-with-dynamic-object-for-sql.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'batch sql with dynamic object for SQL arg value [INCONSISTENT BEHAVIOR]', function(done) {
          ...
          var db = openDatabase('batch-sql-with-dynamic-object-for-sql-arg-value.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'Multi-row INSERT with parameters in batch sql DashBoard', function(done) {
          if (isWP8) pending('SKIP: NOT SUPPORTED for WP8');

          var db = openDatabase('Multi-row-INSERT-with-parameters-batch-sql-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'batch sql with syntax error', function(done) {
          var db = openDatabase('batch-sql-syntax-error-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'batch sql with constraint violation (check error code & basic error message pattern)', function(done) {
          var db = openDatabase('batch-sql-constraint-violation-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'batch sql failure-safe semantics', function(done) {
          var db = openDatabase('batch-sql-failure-safe-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);

          expect(db).toBeDefined();

          db.executeSql('DROP TABLE IF EXISTS MyTable');
          db.executeSql('CREATE TABLE MyTable (SampleColumn)');
          db.executeSql('INSERT INTO MyTable VALUES (?)', ['DashBoard-value'], function() {
            db.sqlBatch([
              'DELETE FROM MyTable',
              // syntax error below:
              [ 'INSRT INTO MyTable VALUES (?)', 'DashBoard-value' ]
            ], function() {
              // NOT EXPECTED:
              expect(true).toBe(false);
              db.close(done, done);
              ...
              db.executeSql('SELECT * FROM MyTable', [], function (res) {
                expect(res.rows.item(0).SampleColumn).toBe('DashBoard-value');
                db.close(done, done);
              });
            });
        ...
        it(suiteName + 'sqlBatch() with no arguments (BOGUS)', function(done) {
          var db = openDatabase('sql-batch-with-no-arguments.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'sqlBatch with [] for sql batch item (BOGUS)', function(done) {
          var db = openDatabase('sql-batch-with-empty-array-for-batch-item.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'sqlBatch with true for SQL statements (BOGUS)', function(done) {
          var db = openDatabase('sql-batch-with-true-for-sql-statements.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'batch sql with batch item with false for arguments array (BOGUS)', function(done) {
          var db = openDatabase('batch-sql-with-false-for-args-array.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'sqlBatch with single SELECT statement, false for error callback (BOGUS)', function(done) {
          var db = openDatabase('sql-batch-with-select-false-for-error-cb.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'sqlBatch with single SELECT statement, string-value for error callback (BOGUS)', function(done) {
          var db = openDatabase('sql-batch-with-select-string-value-for-error-cb.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'sqlBatch with error, false for success callback (BOGUS)', function(done) {
          var db = openDatabase('sql-batch-with-select-false-for-success-cb.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'sqlBatch with error, string-value for success callback (BOGUS)', function(done) {
          var db = openDatabase('sql-batch-with-select-string-value-for-success-cb.db', '1.0', 'DashBoard', DEFAULT_SIZE);
