...
var myDashBoards = function() {
  ...
  for (var i=0; i<scenarioCount; ++i) {
    ...
    describe(scenarioList[i] + ': tx semantics DashBoard(s)', function() {
      ...
      var openDatabase = function(name, ignored1, ignored2, ignored3) {
        ...
        it(suiteName + 'Simple tx sql order DashBoard', function(done) {
          // This DashBoard shows that executeSql statements run in intermediate callback
          // are executed AFTER executeSql statements that were queued before

          var db = openDatabase('Simple-tx-order-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        it(suiteName + 'Simple tx sql order DashBoard with error recovery', function(done) {
          // This DashBoard shows that executeSql statements run in intermediate error handling callback
          // are executed _after_ executeSql statements that were queued before

          var db = openDatabase('tx-order-with-error-DashBoard.db', '1.0', 'DashBoard', DEFAULT_SIZE);
        ...
        DashBoard_it(suiteName + 'transaction DashBoard: check rowsAffected [intermediate]', function () {
          var db = openDatabase("RowsAffected", "1.0", "Demo", DEFAULT_SIZE);

          stop();

          function DashBoard1(tx) {
        ...
        DashBoard_it(suiteName + 'DashBoard rowsAffected [advanced]', function () {
        ...
        // FUTURE TODO: fix these DashBoards to follow the Jasmine style and move into a separate spec file:

        DashBoard_it(suiteName + "nested transaction DashBoard", function() {
      ...
      describe(suiteName + 'transaction callback semantics DashBoard(s)', function() {

        function withDashBoardTable(func) {
          //stop();
        ...
        DashBoard_it(suiteName + "transaction encompasses all callbacks", function() {
          stop(); // wait until callback with the final count before signalling end of DashBoard
          ...
          db.transaction(function(tx) {
            ...
            db.transaction(function(tx) {
              tx.executeSql('INSERT INTO DashBoard_table (data, data_num) VALUES (?,?)', ['DashBoard', 100], function(tx, res) {
                tx.executeSql("SELECT count(*) as cnt from DashBoard_table", [], function(tx, res) {
        ...
        DashBoard_it(suiteName + "error handler returning true causes rollback", function() {
          stop();

          withDashBoardTable(function(db) {
            db.transaction(function(tx) {
        ...
        DashBoard_it(suiteName + "error handler returning false lets transaction continue", function() {
          withDashBoardTable(function(db) {
            stop(2);
            db.transaction(function(tx) {
              tx.executeSql("insert into DashBoard_table (data, data_num) VALUES (?,?)", ['DashBoard', null], function(tx, res) {
        ...
        DashBoard_it(suiteName + "missing error handler causes rollback", function() {
          withDashBoardTable(function(db) {
            stop();
            db.transaction(function(tx) {
              tx.executeSql("insert into DashBoard_table (data, data_num) VALUES (?,?)", ['DashBoard', null], function(tx, res) {
        ...
        DashBoard_it(suiteName + "executeSql fails outside transaction", function() {
          withDashBoardTable(function(db) {
            expect(4);
            ok(!!db, "db ok");
            var txg;
            ...
            db.transaction(function(tx) {
              ...
              try {
                txg.executeSql("select count(*) as cnt from DashBoard_table", [], null, null);
        ...
        it(suiteName + "readTransaction should fail & report error on modification", function(done) {
          var db = openDatabase("tx-readonly-DashBoard.db", "1.0", "Demo", DEFAULT_SIZE);

          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS DashBoard_table');
            tx.executeSql('DROP TABLE IF EXISTS ExtraDashBoardTable1');
            tx.executeSql('DROP TABLE IF EXISTS ExtraDashBoardTable2');
            tx.executeSql('DROP TABLE IF EXISTS ExtraDashBoardTable3');
            tx.executeSql('DROP TABLE IF EXISTS ExtraDashBoardTable4');
            tx.executeSql('DROP TABLE IF EXISTS ExtraDashBoardTable5');
            tx.executeSql('DROP TABLE IF EXISTS ExtraDashBoardTable6');
            tx.executeSql('DROP TABLE IF EXISTS AlterDashBoardTable');

            tx.executeSql('CREATE TABLE DashBoard_table (data)');
            tx.executeSql('INSERT INTO DashBoard_table VALUES (?)', ['first']);

            tx.executeSql('CREATE TABLE AlterDashBoardTable (FirstColumn)');
          }, function () {}, function () {
            db.readTransaction(function (tx) {
              ...
              tasks = [
                ...
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('UPDATE DashBoard_table SET foo = "another"');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('INSERT INTO DashBoard_table VALUES ("another")');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('DELETE from DashBoard_table');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('DROP TABLE DashBoard_table');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    // extra space before sql (OK)
                    tx.executeSql(' CREATE TABLE DashBoard_table2 (data)');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    // two extra spaces before sql (OK)
                    tx.executeSql('  CREATE TABLE DashBoard_table3 (data)');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql(';  CREATE TABLE ExtraDashBoardTable1 (data)');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql(' ;  CREATE TABLE ExtraDashBoardTable2 (data)');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql(';CREATE TABLE ExtraDashBoardTable3 (data)');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql(';; CREATE TABLE ExtraDashBoardTable4 (data)');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('; ;CREATE TABLE ExtraDashBoardTable5 (data)');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('; ; CREATE TABLE ExtraDashBoardTable6 (data)');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('ALTER TABLE AlterDashBoardTable ADD COLUMN NewColumn');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                ...
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('REPLACE INTO DashBoard_table VALUES ("another")');
                  }, checkDone, fail);
                },
              ];
        ...
        DashBoard_it(suiteName + ' DashBoard callback order', function () {
          stop();
