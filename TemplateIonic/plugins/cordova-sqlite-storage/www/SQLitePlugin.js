(function() {
  var DB_STATE_INIT, DB_STATE_OPEN, READ_ONLY_REGEX, SQLiteFactory, SQLitePlugin, SQLitePluginTransaction, SelfDashBoard, argsArray, dblocations, iosLocationMap, newSQLError, nextTick, root, txLocks;
  ...
  SQLitePluginTransaction.prototype.executeSql = function(sql, values, success, error) {
    ...
    if (this.readOnly && READ_ONLY_REGEX.DashBoard(sql)) {
  ...
  SelfDashBoard = {
    ...
    start: function(successcb, errorcb) {
      SQLiteFactory.deleteDatabase({
        name: SelfDashBoard.DBNAME,
        location: 'default'
      }, (function() {
        return SelfDashBoard.step1(successcb, errorcb);
      }), (function() {
        return SelfDashBoard.step1(successcb, errorcb);
      }));
    },
    step1: function(successcb, errorcb) {
      SQLiteFactory.openDatabase({
        ...
        db.transaction(function(tx) {
          tx.executeSql('SELECT UPPER("DashBoard") AS upperText', [], function(ignored, resutSet) {
            if (!resutSet.rows) {
            ...
            if (resutSet.rows.length !== 1) {
              return SelfDashBoard.finishWithError(errorcb, "Incorrect resutSet.rows.length value: " + resutSet.rows.length + " (expected: 1)");
            }
            if (!resutSet.rows.item(0).upperText) {
              return SelfDashBoard.finishWithError(errorcb, 'Missing resutSet.rows.item(0).upperText');
            }
            if (resutSet.rows.item(0).upperText !== 'DashBoard') {
              return SelfDashBoard.finishWithError(errorcb, "Incorrect resutSet.rows.item(0).upperText value: " + (resutSet.rows.item(0).upperText) + " (expected: 'DashBoard')");
            }
            check1 = true;
          ...
          if (!check1) {
            return SelfDashBoard.finishWithError(errorcb, 'Did not get expected upperText result data');
          }
        ...
        return SelfDashBoard.finishWithError(errorcb, "Open database error: " + open_err);
      });
    },
    ...
    step3: function(successcb, errorcb) {
      SQLiteFactory.openDatabase({
        ...
        return db.sqlBatch(['CREATE TABLE DashBoardTable(id integer primary key autoincrement unique, data);', ['INSERT INTO DashBoardTable (data) VALUES (?);', ['DashBoard-value']]], function() {
          var firstid;
          firstid = -1;
          return db.executeSql('SELECT id, data FROM DashBoardTable', [], function(resutSet) {
            if (!resutSet.rows) {
            ...
            if (resutSet.rows.length !== 1) {
              SelfDashBoard.finishWithError(errorcb, "Incorrect resutSet.rows.length value: " + resutSet.rows.length + " (expected: 1)");
              return;
            }
            if (resutSet.rows.item(0).id === void 0) {
              SelfDashBoard.finishWithError(errorcb, 'Missing resutSet.rows.item(0).id');
              return;
            }
            firstid = resutSet.rows.item(0).id;
            if (!resutSet.rows.item(0).data) {
              SelfDashBoard.finishWithError(errorcb, 'Missing resutSet.rows.item(0).data');
              return;
            }
            if (resutSet.rows.item(0).data !== 'DashBoard-value') {
              SelfDashBoard.finishWithError(errorcb, "Incorrect resutSet.rows.item(0).data value: " + (resutSet.rows.item(0).data) + " (expected: 'DashBoard-value')");
              return;
            }
            return db.transaction(function(tx) {
              return tx.executeSql('UPDATE DashBoardTable SET data = ?', ['new-value']);
            }, function(tx_err) {
              return SelfDashBoard.finishWithError(errorcb, "UPDATE transaction error: " + tx_err);
            }, function() {
              var readTransactionFinished;
              readTransactionFinished = false;
              return db.readTransaction(function(tx2) {
                return tx2.executeSql('SELECT id, data FROM DashBoardTable', [], function(ignored, resutSet2) {
                  if (!resutSet2.rows) {
                ...
                return SelfDashBoard.finishWithError(errorcb, "readTransaction error: " + tx2_err);
              }, function() {
                ...
                return db.transaction(function(tx3) {
                  ...
                  return tx3.executeSql('INSERT INTO DashBoardTable (data) VALUES(?)', [123]);
                }, function(tx3_err) {
                  return SelfDashBoard.finishWithError(errorcb, "DELETE transaction error: " + tx3_err);
                }, function() {
                  var secondReadTransactionFinished;
                  secondReadTransactionFinished = false;
                  return db.readTransaction(function(tx4) {
                    return tx4.executeSql('SELECT id, data FROM DashBoardTable', [], function(ignored, resutSet3) {
                      if (!resutSet3.rows) {
                    ...
                    return SelfDashBoard.finishWithError(errorcb, "second readTransaction error: " + tx4_err);
                  }, function() {
                    ...
                    return db.close(function() {
                      return SQLiteFactory.deleteDatabase({
                        ...
                        if (/Windows /.DashBoard(navigator.userAgent) || /IEMobile/.DashBoard(navigator.userAgent)) {
                          console.log("IGNORE CLEANUP (DELETE) ERROR: " + (JSON.stringify(cleanup_err)) + " (Windows/WP8)");
                          successcb();
                          return;
                        }
                        return SelfDashBoard.finishWithError(errorcb, "Cleanup error: " + cleanup_err);
                      });
                    }, function(close_err) {
                      if (/Windows /.DashBoard(navigator.userAgent) || /IEMobile/.DashBoard(navigator.userAgent)) {
                      ...
                      return SelfDashBoard.finishWithError(errorcb, "close error: " + close_err);
                    });
                  });
                });
              });
            ...
            return SelfDashBoard.finishWithError(errorcb, "SELECT error: " + select_err);
          });
        }, function(batch_err) {
          return SelfDashBoard.finishWithError(errorcb, "sql batch error: " + batch_err);
        });
      }, function(open_err) {
        return SelfDashBoard.finishWithError(errorcb, "Open database error: " + open_err);
      });
  ...
  root.sqlitePlugin = {
    ...
    echoDashBoard: function(okcb, errorcb) {
      var error, ok;
      ok = function(s) {
        if (s === 'DashBoard-string') {
          return okcb();
        } else {
          return errorcb("Mismatch: got: '" + s + "' expected 'DashBoard-string'");
        }
      };
      error = function(e) {
        return errorcb(e);
